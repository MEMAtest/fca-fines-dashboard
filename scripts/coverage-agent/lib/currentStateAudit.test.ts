...
describe("current RegActions state audit", () => {
  it("captures required routes, live hubs, API dates, canonical and redirect behaviour", async () => {
    const fetchMock = vi.fn(async (input: string | URL) => {
      ...
      if (url.includes("/api/site/unified/search")) return new Response(JSON.stringify({ results: [{ date_issued: "2026-08-12" }] }), { status: 200, headers: { "content-type": "application/json" } });
    ...
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes("/api/site/unified/search?regulator=FCA"))).toBe(true);
  });
  ...
  it("records fetch failures instead of throwing so downstream QA artifacts can still be built", async () => {
    const fetchMock = vi.fn(async (input: string | URL) => {
      ...
      if (url.includes("/api/site/unified/search")) return new Response("temporarily down", { status: 503 });
  ...
  it("bounds concurrent checks so the audit cannot exhaust production database connections", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL) => {
      ...
      active -= 1;
      const url = String(input);
      return new Response(url.includes("/api/site/unified/search") ? JSON.stringify({ results: [] }) : html("Healthy"), {
        status: 200,
