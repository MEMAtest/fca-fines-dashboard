...
import type { FineRecord, StatsResponse, NotificationItem } from "../types.js";
...
import { MonthlyComparisonChart } from "../components/MonthlyComparisonChart.js";
import { MobileNav } from "../components/MobileNav.js";
import { Modal } from "../components/Modal.js";
import { BreachByTypeChart } from "../components/BreachByTypeChart.js";
import { RegulatorImpactChart } from "../components/RegulatorImpactChart.js";
...
import {
  ...
  CURRENT_YEAR,
} from "../hooks/useDashboardState.js";
import { useFinesData, type TrendPoint } from "../hooks/useFinesData.js";
...
const LazyAlertSubscribeModal = lazy(() =>
  import("../components/AlertSubscribeModal.js").then((module) => ({
    default: module.AlertSubscribeModal,
  })),
);
const LazyComparisonView = lazy(() =>
  import("../components/ComparisonView.js").then((module) => ({
  ...
  if (records.length) {
    ...
    if (latest) {
      items.push({
        ...
        time: new Date(latest.date_issued).toLocaleDateString("en-GB", {
          month: "short",
          day: "numeric",
        }),
      });
    ...
    largeFines.forEach((record) => {
      items.push({
        ...
        time: new Date(record.date_issued).toLocaleDateString("en-GB", {
          month: "short",
          day: "numeric",
        }),
      });
    });
    const groupedByMonth = records.reduce<
...
export function Dashboard() {
  ...
  const { fines, stats, loading, error } = useMultiRegulator
    ? unifiedData
    : fcaData;
  ...
  const [readNotifications, setReadNotifications] = useLocalStorage<string[]>(
    "fca-notifications-read",
    [],
  );
  const notifications = useMemo(
    () =>
      notificationsRaw.map((item: NotificationItem) => ({
        ...item,
  ...
  const timeline = useMemo(() => {
    const entries = (Object.values(trendsByYear) as TrendPoint[][]).flat();
    return entries.sort((a: TrendPoint, b: TrendPoint) => {
      if (a.year === b.year) return a.period - b.period;
      return a.year - b.year;
    });
  }, [trendsByYear]);
  ...
  const filteredFines = useMemo(() => {
    ...
    if (advancedFilters.breachTypes.length) {
      scoped = scoped.filter((fine) =>
        fine.breach_categories?.some((cat: string) =>
          advancedFilters.breachTypes.includes(cat || ""),
        ),
      );
    }
      ...
      scoped = scoped.filter((fine) => {
        ...
        if (searchScope === "category") {
          return (
            ...
            fine.breach_categories?.some((cat) =>
  ...
  const breachOptions = useMemo(() => {
    ...
    fines.forEach((fine) =>
      fine.breach_categories?.forEach((cat) => cat && set.add(cat)),
    );
  ...
  const activeChips = useMemo(() => {
    ...
    if (regulator !== "All") {
      chips.push({
        label: `Regulator: ${regulator}`,
        onRemove: () => setRegulator("All"),
      });
    }
    if (country !== "All") {
      chips.push({
  ...
  const timelineForChart = useMemo(() => {
    ...
    return filtered.length
      ? filtered
      : timeline.filter((entry) => entry.year === year);
  ...
  const fallbackComparisonYear = useMemo(
    () =>
      availableYears.find((candidateYear) => candidateYear !== primaryYear) ??
      null,
    [availableYears, primaryYear],
  );
  ...
  const comparisonCategoryOptions = useMemo(() => {
    ...
    comparisonRecords.forEach((record) =>
      record.breach_categories?.forEach((cat: string) => cat && set.add(cat)),
    );
    return Array.from(set).sort();
  }, [comparisonRecords]);
  const comparisonReady =
    !!recordsByYear[primaryYear] &&
    (!comparisonYear ||
      comparisonYear === primaryYear ||
      !!recordsByYear[comparisonYear]);
  useEffect(() => {
    setComparisonCategories((prev: string[]) =>
      prev.filter((selectedCategory: string) =>
        comparisonCategoryOptions.includes(selectedCategory),
      ),
    );
  ...
  function handleTimelineRange(
    ...
    const startDate = startOfMonth(
      new Date(start.year ?? primaryYear, (start.period ?? 1) - 1, 1),
    );
    const endDate = endOfMonth(
      new Date(end.year ?? primaryYear, (end.period ?? 1) - 1, 1),
    );
  ...
  function markAllNotificationsRead() {
    setReadNotifications((prev: string[]) => {
      ...
      notificationsRaw.forEach((item: NotificationItem) => next.add(item.id));
  ...
  async function handleNotificationsRefresh() {
    try {
      ...
      setToast({
        message: success ? "Alerts refreshed" : "Unable to refresh alerts",
  ...
  return (
    <div className="app-shell">
      <HeroStats
        ...
        onTotalClick={() =>
          openRecordsModal(
            "Total fines drilldown",
            year === 0 ? "2013 – Today" : `${year} snapshot`,
            filteredFines,
          )
        }
            ...
            <LazyVisible fallback={<div style={{ minHeight: 300 }} />}>
              <div className="grid grid--two-col">
                <MonthlyComparisonChart
                  ...
                  loading={
                    !recordsByYear[primaryYear] ||
                    (comparisonYear !== null && !recordsByYear[comparisonYear])
                  }
                />
                <div className="panel">
                  <button
      ...
      {advancedOpen && (
        <Suspense fallback={null}>
          <LazyAdvancedFilters
            ...
            currentYear={primaryYear}
            onApply={(values: AdvancedFilterValues) =>
        ...
        >
          <Suspense fallback={<p className="status">Loading comparison…</p>}>
            <LazyComparisonView
              ...
              onPresetCategories={(categories: string[]) =>
                setComparisonCategories(categories)
              }
              onNotify={(message: string, type?: "success" | "error") =>
                setToast({ message, type })
              }
      ...
      <MobileNav
        ...
        onCompare={() => {
          ...
          const effectiveComparisonYear =
            comparisonYear ?? fallbackComparisonYear;
        ...
        compareEnabled={
          !useMultiRegulator &&
          Boolean(comparisonYear ?? fallbackComparisonYear)
        }
