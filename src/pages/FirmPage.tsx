...
export function FirmPage() {
  ...
  useEffect(() => {
    const firmSlug = slug;
    if (typeof firmSlug !== "string") return;
    const stableFirmSlug: string = firmSlug;
    let mounted = true;
    async function load() {
      setLoading(true);
      setError(null);
      try {
        const res = await fetchFirm(stableFirmSlug, 250);
        if (!mounted) return;
        setFirm(res.data);
      } catch (e) {
        console.error(e);
        if (mounted)
          setError("Unable to load this firm right now. Please try again.");
      } finally {
  ...
  const breachSummary = useMemo(() => {
    ...
    firm.records.forEach((r) => {
      const labels = r.breach_categories?.length
        ? r.breach_categories
        : ["Unclassified"];
            ...
            <div className="hub-card" style={{ marginBottom: "1rem" }}>
              <div className="hub-card__meta">
                ...
                <span className="hub-chip hub-chip--neutral">
                  {currency.format(firm.totalAmount)}
                </span>
                <span className="hub-chip hub-chip--neutral">
              ...
              <h3>At A Glance</h3>
              <p>
                Open this entity in the dashboard to compare with peers, filter
                by year, and export records.
              </p>
            </div>
