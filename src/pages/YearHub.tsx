...
import { fetchFines, fetchYears } from "../api.js";
...
export function YearHub() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const [yearsRes, finesRes] = await Promise.all([
          fetchYears(),
          fetchFines(year),
        ]);
        if (!mounted) return;
  ...
  const totals = useMemo(
    () => ({ count: records.length, amount: sumAmount(records) }),
    [records],
  );
  const topPenalties = useMemo(
    () => [...records].sort((a, b) => b.amount - a.amount).slice(0, 10),
    [records],
  );
  const topFirms = useMemo(() => {
    const map = new Map<
      string,
      { firm: string; total: number; count: number }
    >();
    records.forEach((r) => {
  ...
  const annualReviewSlug = useMemo(() => {
    ...
    if (year >= 2013 && year <= 2025)
      return `/blog/fca-fines-${year}-annual-review`;
    return null;
  }, [year]);
      ...
      <div className="hub-container">
        <header className="hub-hero">
          ...
          <div className="hub-hero__actions">
            <Link
              to={`/dashboard?year=${encodeURIComponent(String(year))}`}
              className="btn btn-primary"
            >
              View {year} in Dashboard
            </Link>
            <Link to="/years" className="btn btn-ghost">
              All Years
            </Link>
            {annualReviewSlug && (
              <Link to={annualReviewSlug} className="btn btn-ghost">
                Read Annual Review
              </Link>
            )}
            {year === 2026 && (
            ...
            <div className="hub-card" style={{ marginBottom: "1rem" }}>
              <div className="hub-card__meta">
                <span className="hub-chip">
                  {summary?.fineCount ?? totals.count} actions
                </span>
                <span className="hub-chip hub-chip--neutral">
                  {currency.format(summary?.totalAmount ?? totals.amount)}
                </span>
              </div>
              <h3>At A Glance</h3>
              <p>
                Largest penalties and top entities for {year}. Click into the
                dashboard to compare with other years.
              </p>
            </div>
                ...
                <tbody>
                  {topPenalties.map((r) => (
                    ...
                    >
                      ...
                      <td>
                        <EvidenceTrigger className="hub-link hub-link--button" evidence={buildFineRecordEvidence(r, "year_hub")}>View evidence</EvidenceTrigger>
                      </td>
                    </tr>
                  ))}
                </tbody>
