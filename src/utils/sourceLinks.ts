...
export interface SourceLinkRecord {
  ...
  source_link_status?: SourceLinkStatus | null;
...
function isSyntheticCnmvDetail(url: string) {
  return /cnmv\.es\/portal\/consultas\/registrosanciones\/s\/\d{4}\//i.test(
    url,
  );
}
...
function getPreferredOfficialLink(
  record: Pick<
    SourceLinkRecord,
    "detail_url" | "official_publication_url" | "final_notice_url"
  >,
) {
  return (
    normalizeUrl(record.detail_url) ||
    normalizeUrl(record.official_publication_url) ||
    normalizeUrl(record.final_notice_url)
  );
}
...
export function getRecordSourceStatus(
  ...
  return (
    ...
    deriveSourceLinkStatus(
      record.regulator,
      getPreferredOfficialLink(record),
      record.listing_url || record.source_url,
    )
  );
}
...
export function getRecordListingUrl(
  record: Pick<SourceLinkRecord, "regulator" | "source_url" | "listing_url">,
) {
  const directListing =
    normalizeUrl(record.listing_url) || normalizeUrl(record.source_url);
  if (directListing) {
    return directListing;
  }
  return (
    getRegulatorCoverage(record.regulator)?.officialSources[0]?.url ?? null
  );
}
