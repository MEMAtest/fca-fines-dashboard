...
export function RegulatorImpactChart({
  records,
  exportId,
}: RegulatorImpactChartProps) {
  ...
  const aggregates = useMemo(() => {
    ...
    records.forEach((record) => {
      ...
      const regulator = regulatorMap.get(regulatorKey) ?? {
        amount: 0,
        count: 0,
        breakdown: {},
      };
      ...
      regulator.count += 1;
      regulator.breakdown[categoryKey] = regulator.breakdown[categoryKey] ?? {
        amount: 0,
        count: 0,
      };
      ...
      categoryTotals.set(
        categoryKey,
        (categoryTotals.get(categoryKey) ?? 0) + 1,
      );
    });
          ...
          <BarChart data={chartData} layout="vertical" barCategoryGap={18}>
            ...
            <XAxis
              ...
              domain={[
                0,
                (dataMax) =>
                  mode === "share" ? Math.max(100, dataMax) : dataMax * 1.1,
              ]}
            />
            <YAxis
              type="category"
            ...
            {series.map((seriesName, index) => (
              <Bar
                ...
                stackId="regulators"
                fill={
                  seriesName === STACK_OTHER
                    ? "#94a3b8"
                    : colorPalette[index % colorPalette.length]
                }
                radius={
                  index === series.length - 1 ? [0, 10, 10, 0] : [0, 0, 0, 0]
                }
              />
            ))}
          </BarChart>
