...
type SavedCategoryPreset = { name: string; categories: string[] };
...
export function ComparisonView({
  ...
  const scopedRecords = useMemo(
    () =>
      records.filter(
        (record) =>
          record.year_issued === primaryYear ||
          record.year_issued === comparisonYear,
      ),
  ...
  const filteredRecords = useMemo(() => {
    ...
    return scopedRecords.filter((record) =>
      record.breach_categories?.some((category: string) =>
        selectedCategories.includes(category || "Unclassified"),
      ),
    );
  ...
  const topPrimary = useMemo(
    () => buildTopFines(filteredRecords, primaryYear),
    [filteredRecords, primaryYear],
  );
  const topComparison = useMemo(
    () => buildTopFines(filteredRecords, comparisonYear),
    [filteredRecords, comparisonYear],
  );
  ...
  function handleSaveCategoryPreset() {
    ...
    if (!name) return;
    setSavedCategoryPresets((prev: SavedCategoryPreset[]) => {
      const next = [
        { name, categories: selectedCategories },
        ...prev.filter((preset: SavedCategoryPreset) => preset.name !== name),
      ];
  ...
  const chartAxisFormatter =
    trendMetric === "amount"
      ? (value: number) => `£${Math.round(value / 1_000_000)}m`
      : (value: number) => `${value}`;
  const chartValueFormatter =
    trendMetric === "amount"
      ? (value: number) => `£${Number(value).toLocaleString()}`
      : (value: number) => `${value} notices`;
  const focusedMonthData = focusedMonth
    ? chartData.find((row) => row.month === focusedMonth)
    : null;
    ...
    <div className="comparison-view">
      <div className="comparison-view__hero">
        <p className="comparison-view__intro">
          Track cadence, drill into breach families, and share this custom lens
          with one click.
        </p>
      ...
      <div className="comparison-view__chips">
        ...
        <span className="chip chip--ghost chip--static">
          {mode === "lines" ? "Trend lines enabled" : "Variance bars enabled"}
        </span>
      </div>
      <div className="comparison-controls comparison-controls--enhanced">
        ...
        <div className="comparison-controls__toggles">
          ...
          <div
            className="comparison-view__mode-switch"
            role="group"
      ...
      <div className="comparison-summary-grid">
        {summary.map((item) => (
          <article key={item.year} className="comparison-summary-card">
            ...
            <p>
              {item.count} notices • avg £
              {Math.round(item.average).toLocaleString("en-GB")}
            </p>
            {item.deltaLabel && (
              <span
                className={`stat-card__chip ${item.deltaLabel.includes("-") ? "stat-card__chip--down" : ""}`}
              >
                {item.deltaLabel}
              </span>
            )}
      ...
      <div className="comparison-categories">
        ...
        <div className="comparison-presets__toolbar">
          <button
            ...
            onClick={handleSaveCategoryPreset}
        ...
        {savedCategoryPresets.length > 0 && (
          <div className="comparison-presets__saved">
            ...
            <div className="preset-list">
              {savedCategoryPresets.map((preset: SavedCategoryPreset) => (
        ...
        <div className="chip-grid">
          <button
            type="button"
            className={
              selectedCategories.length === 0 ? "chip chip--active" : "chip"
            }
          ...
          <div className="comparison-table__header">
            <div>
              ...
              <p className="panel__hint">
                {categorySummary.length} breach families compared • showing top
                8 by current impact.
              </p>
            </div>
                  ...
                  onClick={() => {
                    try {
                      exportCategorySummary(
                        categorySummary,
                        `category-summary-${primaryYear}-vs-${comparisonYear}`,
                        "csv",
                      );
      ...
      {loading ? (
        ...
        <>
            ...
            <ResponsiveContainer width="100%" height={240}>
              {mode === "lines" ? (
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value: any) =>
                      chartValueFormatter(value as number)
                    }
                  />
                  <Line
                    type="monotone"
          ...
          <div className="panel__meta">
            {focusedMonthData ? (
              <p>
                Focused on <strong>{focusedMonthData.month}</strong> •{" "}
                {primaryYear}: {chartValueFormatter(focusedMonthData.primary)}{" "}
                vs {comparisonYear}:{" "}
                {chartValueFormatter(focusedMonthData.comparison)}
              </p>
            ) : (
              <p>
                Click any point on the chart to lock a month for deeper context.
              </p>
            )}
          </div>
          ...
          {radarData.length > 0 && (
            <div className="comparison-radar">
              <div>
                ...
                <p>
                  Track how each breach family contributes to enforcement totals
                  for both years.
                </p>
              </div>
...
function renderTopList(records: FineRecord[]) {
  ...
  return records.map((record) => (
    <article
      ...
      className="comparison-view__card"
    >
      <h5>{record.firm_individual}</h5>
      <p>
        {new Date(record.date_issued).toLocaleDateString("en-GB")} • £
        {safeNum(record.amount).toLocaleString("en-GB")}
      </p>
      <span className="badge">
        {record.breach_categories?.[0] || "Unclassified"}
      </span>
    </article>
  ));
}
...
function buildSummary(
  ...
  const delta =
    comparison.total > 0
      ? ((primary.total - comparison.total) / comparison.total) * 100
      : null;
  return [
    {
      ...primary,
      deltaLabel:
        delta != null
          ? `${delta >= 0 ? "+" : ""}${delta.toFixed(1)}% vs ${comparisonYear}`
          : null,
    },
    {
      ...comparison,
      deltaLabel:
        delta != null
          ? `${-delta >= 0 ? "+" : ""}${(-delta).toFixed(1)}% vs ${primaryYear}`
          : null,
    },
  ];
}
...
function buildCategoryRadar(
  ...
  records.forEach((record) => {
    ...
    (record.breach_categories || ["Unclassified"]).forEach(
      (category: string) => {
