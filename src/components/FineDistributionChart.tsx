...
interface InteractiveChartState {
  activePayload?: ChartStatePayload[];
}

export function FineDistributionChart({
  records,
  onSelectRange,
  exportId,
}: FineDistributionChartProps) {
  ...
  const baseBuckets = useMemo(() => {
    return AMOUNT_BUCKETS.map((bucket) => {
      const bucketRecords = records.filter(
        (record) => record.amount >= bucket.min && record.amount < bucket.max,
      );
  ...
  const medianBucketLabel = useMemo(() => {
    ...
    const bucket = AMOUNT_BUCKETS.find(
      (range) => medianAmount >= range.min && medianAmount < range.max,
    );
  ...
  const stackedData = useMemo(() => {
    return baseBuckets.map((bucket) => {
      ...
      topCategories.forEach((category) => {
        ...
        row[category] =
          statMode === "percentage" && bucket.count > 0
    ...
    <div className="panel panel--tall" id={panelId}>
      ...
      <div className="panel__toolbar panel__toolbar--stacked">
        ...
        <div
          className="panel__toolbar-buttons"
          role="group"
          aria-label="Distribution layout"
        >
          <button
            type="button"
      ...
      <p className="panel__description">
        Tap a band to drill into those fines. Median notice currently sits in{" "}
        <strong>{medianBucketLabel}</strong>.
      </p>
      <div className="panel__chart panel__chart--compact">
        <ResponsiveContainer width="100%" height={240}>
          <BarChart
            ...
            onClick={(state) =>
              handleBucketSelect(
                (state as InteractiveChartState | undefined)
                  ?.activePayload?.[0],
              )
            }
            onMouseMove={(state) =>
              handleActiveState(
                (state as InteractiveChartState | undefined)?.activePayload,
              )
            }
            ...
            <Tooltip
              ...
              formatter={(value: any, name: string, props) => {
                if (!stacked) {
                  if (statMode === "count") {
                    return [
                      `${value} notices (${props?.payload?.percentage?.toFixed(1)}%)`,
                      "Total",
                    ];
                  }
      ...
      <div className="panel__meta">
        {focusRange && focusedBucket ? (
          ...
          <p>
            Medians and interactive stacks help explain where enforcement volume
            concentrates.
          </p>
        )}
      </div>
