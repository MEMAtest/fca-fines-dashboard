...
type EnrichedFineRecord = FineRecord & { formattedDate: string };

export interface FineTotalsModalProps {
  open: boolean;
  records: FineRecord[];
  year: number;
  title?: string;
...
export function FineTotalsModal({
  open,
  records,
  year,
  title,
  ...
  const resolvedSubtitle =
    subtitle ?? (year === 0 ? "2013 – Today" : `${year} snapshot`);

  const enrichedRecords = useMemo<EnrichedFineRecord[]>(
    () =>
      records.map((record) => ({
        ...record,
  ...
  const visibleColumns = useMemo(
    () => columns.filter((column: ColumnConfig) => column.visible),
    [columns],
  );
  const totalPages = pageSize === 0 ? 1 : Math.ceil(sorted.length / pageSize);
  const currentPage =
    pageSize === 0 ? 0 : Math.min(page, Math.max(totalPages - 1, 0));
  ...
  useEffect(() => {
    ...
    const viewport =
      Math.ceil(container.clientHeight / VIRTUAL_ROW_HEIGHT) + 10;
  ...
  useEffect(() => {
    ...
    function handleScroll() {
      ...
      const startIndex = Math.max(
        0,
        Math.floor(scrollTop / VIRTUAL_ROW_HEIGHT) - 8,
      );
  ...
  const [topSpacer, bottomSpacer] = useMemo(() => {
    ...
    const bottom =
      Math.max(sorted.length - virtualRange.end, 0) * VIRTUAL_ROW_HEIGHT;
    return [top, bottom];
  ...
  const showingStart =
    sorted.length === 0
      ? 0
      : shouldVirtualize
        ? Math.min(virtualRange.start + 1, sorted.length)
  ...
  function toggleColumn(key: ColumnConfig["key"]) {
    setColumns((prev: ColumnConfig[]) => {
      const next = prev.map((column: ColumnConfig) =>
        column.key === key ? { ...column, visible: !column.visible } : column,
      );
      if (next.filter((column: ColumnConfig) => column.visible).length === 0) {
  ...
  function handleColumnsPreset(action: "all" | "minimal" | "reset") {
    setColumns((prev: ColumnConfig[]) => {
      if (action === "all") {
        ...
        return prev.map((column: ColumnConfig) => ({
          ...column,
          visible: true,
        }));
      }
      if (action === "minimal") {
        const focusKeys: ColumnKey[] = [
          "firm_individual",
          "formattedDate",
          "amount",
        ];
        ...
        return prev.map((column: ColumnConfig) => ({
          ...column,
          visible: focusKeys.includes(column.key),
        }));
      }
      onNotify?.("Column defaults restored", "success");
      return DEFAULT_COLUMNS.map((column: ColumnConfig) => ({ ...column }));
    });
  }
  ...
  async function handleExport(format: "csv" | "xlsx") {
    try {
      await exportData({
        ...
        format,
        records: sorted,
        transform: (record: FineRecord) => {
          const row: Record<string, string | number> = {};
          visibleColumns.forEach((column: ColumnConfig) => {
            if (column.key === "amount") {
              row[column.label] = record.amount;
            } else if (column.key === "formattedDate") {
              row[column.label] = new Date(
                record.date_issued,
  ...
  if (!records.length) {
    return (
      ...
      >
        <p className="status">
          Adjust the filters or search to surface fines to explore.
        </p>
      </Modal>
    );
  }
    ...
    <Modal title={resolvedTitle} subtitle={resolvedSubtitle} onClose={onClose}>
          ...
          {columnsOpen && (
            <div className="modal__columns-menu">
              <div className="modal__columns-actions">
                <button
                  type="button"
                  onClick={() => handleColumnsPreset("all")}
                >
                  Show all
                </button>
                <button
                  type="button"
                  onClick={() => handleColumnsPreset("minimal")}
                >
                  Focus view
                </button>
                <button
                  type="button"
                  onClick={() => handleColumnsPreset("reset")}
                >
                  Reset
                </button>
              </div>
      ...
      >
        <table>
          ...
          <tbody>
            ...
            {visibleRows.map((record: EnrichedFineRecord) => {
              const rowKey =
                record.fine_reference ||
                `${record.firm_individual}-${record.date_issued}`;
              return (
                <tr key={rowKey}>
                  {visibleColumns.map((column: ColumnConfig) => (
                    <td key={column.key}>
            ...
            {!visibleRows.length && (
              <tr>
                <td colSpan={visibleColumns.length + 1}>
                  <div className="modal__table-empty">
                    No records match this slice. Try adjusting the filters.
                  </div>
                </td>
              </tr>
            )}
      ...
      <div className="modal__pagination">
        <div className="modal__pagination-controls">
          ...
          <span>
            Page {pageSize === 0 ? "∞" : totalPages ? currentPage + 1 : 0} of{" "}
            {pageSize === 0 ? "∞" : totalPages}
          </span>
          <button
            type="button"
            onClick={() =>
        ...
        <div className="modal__pagination-meta">
          <span>
            Showing {showingStart || 0} – {showingEnd || 0} of {sorted.length}{" "}
            records
          </span>
          <label>
            Rows per page
      ...
      <p className="modal__hint">
        {filtered.length} records match your filters. Refine filters or search
        to focus further.
      </p>
    </Modal>
  );
...
function SortIndicator({
  ...
  if (activeKey !== resolvedKey) {
    return (
      <span className="modal__sort-indicator" aria-hidden="true">
        ↕
      </span>
    );
  }
  return (
    <span
      className="modal__sort-indicator modal__sort-indicator--active"
      aria-hidden="true"
    >
      {direction === "asc" ? "↑" : "↓"}
    </span>
