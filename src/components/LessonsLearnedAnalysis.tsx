...
export function LessonsLearnedAnalysis({ records, year }: LessonsLearnedProps) {
  ...
  const sortedRecords = useMemo(() => {
    ...
    if (filterCategory !== "all") {
      filtered = filtered.filter((r) =>
        r.breach_categories?.includes(filterCategory),
      );
    }
  ...
  const metrics = useMemo(() => {
    ...
    records.forEach((r) => {
      firmCounts.set(
        r.firm_individual,
        (firmCounts.get(r.firm_individual) || 0) + 1,
      );
    });

    const repeatOffenders = Array.from(firmCounts.entries()).filter(
      ([_, count]) => count > 1,
    );
    ...
    return {
      repeatOffenderRate: firmCounts.size
        ? (repeatOffenders.length / firmCounts.size) * 100
        : 0,
      repeatOffenders: repeatOffenders.length,
      avgFine,
      largeFineCount: largeFines.length,
      largeFinePercentage: records.length
        ? (largeFines.length / records.length) * 100
        : 0,
    };
  }, [records]);
  ...
  const tabs = [
    {
      id: "summaries" as TabId,
      label: "Fine Summaries",
    ...
    {
      id: "learnings" as TabId,
      label: "What Firms Can Learn",
...
function SummariesTab({
  ...
  return (
    <div className="summaries-tab">
          ...
          return (
            ...
            >
              ...
              <p className="summary-card__date">
                ...
                {record.regulator && (
                  <span className="summary-card__regulator">
                    {" "}
                    • {record.regulator}
                  </span>
                )}
              ...
              {record.breach_categories &&
                record.breach_categories.length > 0 && (
      ...
      {records.length > 20 && (
        <p className="summaries-footer">
          Showing 20 of {records.length} notices
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="themes-tab">
      <p className="themes-intro">
        Analysis of enforcement patterns reveals common compliance failures and
      ...
      <div className="themes-grid">
        {themes.map((theme) => (
          <div
            key={theme.category}
            className={`theme-card theme-card--${theme.riskLevel}`}
          >
    ...
    <div className="learnings-tab">
      ...
      <div className="metrics-row">
        <div className="metric-card">
          ...
          <div>
            <span className="metric-value">
              {metrics.repeatOffenders} Repeat Offenders
            </span>
            <span className="metric-label">
              ({metrics.repeatOffenderRate.toFixed(1)}%)
            </span>
          </div>
        </div>
        <div className="metric-card">
          ...
          <div>
            <span className="metric-value">
              {formatter.format(metrics.avgFine)}
            </span>
        ...
        <div className="metric-card">
          ...
          <div>
            <span className="metric-value">
              {metrics.largeFineCount} Fines Over £10m
            </span>
            <span className="metric-label">
              ({metrics.largeFinePercentage.toFixed(1)}%)
            </span>
          </div>
        </div>
      </div>
      ...
      {/* Lessons */}
      <h4 className="learnings-section-title">
      ...
      <div className="lessons-grid">
        {lessons.map((lesson) => (
          <div
            key={lesson.id}
            className={`lesson-card lesson-card--${lesson.priority}`}
          >
            ...
            <div className="lesson-card__content">
              ...
              <div className="lesson-card__footer">
                ...
                <span className="lesson-frequency">
                  {lesson.frequency} related notices
                </span>
              </div>
            </div>
          </div>
...
function analyzeThemes(records: FineRecord[]): Theme[] {
  const categoryMap = new Map<
    string,
    { count: number; totalAmount: number; summaries: string[] }
  >();

  records.forEach((record) => {
    const cats = record.breach_categories?.length
      ? record.breach_categories
      : ["Unclassified"];
    cats.forEach((cat) => {
  ...
  return Array.from(categoryMap.entries())
    ...
    .map(([category, data]) => ({
      ...
      riskLevel:
        data.totalAmount > 100_000_000
          ? "high"
          : data.totalAmount > 20_000_000
            ? "medium"
            : "low",
    }));
}
...
function extractLessons(records: FineRecord[]): Lesson[] {
  ...
  > = {
    MARKET_ABUSE: {
      ...
      description:
        "Invest in automated surveillance systems capable of detecting unusual trading patterns. Ensure timely escalation of potential market abuse to compliance teams.",
      icon: <Shield size={20} />,
    },
    SYSTEMS_CONTROLS: {
      title: "Enhance Control Frameworks",
      description:
        "Conduct regular reviews of operational controls. Implement robust change management and ensure adequate testing before system changes.",
      icon: <Target size={20} />,
    },
    CLIENT_MONEY: {
      title: "Prioritize Client Asset Protection",
      description:
        "Maintain rigorous segregation of client funds. Implement daily reconciliations and ensure CASS compliance is actively monitored.",
      icon: <Users size={20} />,
    },
    FINANCIAL_PROMOTIONS: {
      title: "Review Marketing Compliance",
      description:
        "Establish clear approval processes for all financial promotions. Ensure risk warnings are prominent and communications are fair, clear and not misleading.",
      icon: <FileText size={20} />,
    },
    REPORTING: {
      title: "Improve Regulatory Reporting",
      description:
        "Automate reporting processes where possible. Implement data quality checks and ensure adequate time for review before submission deadlines.",
      icon: <TrendingUp size={20} />,
    },
    AML: {
      title: "Strengthen AML Controls",
      description:
        "Enhance customer due diligence procedures. Regularly tune transaction monitoring systems and ensure timely filing of suspicious activity reports.",
      icon: <AlertTriangle size={20} />,
    },
    GOVERNANCE: {
      title: "Enhance Board Oversight",
      description:
        "Ensure boards receive adequate management information on compliance risks. Implement clear escalation paths for material issues.",
      icon: <Users size={20} />,
    },
    FRAUD: {
      title: "Prevent and Detect Fraud",
      description:
        "Implement robust fraud prevention controls. Establish clear procedures for identifying and reporting suspicious activity.",
      icon: <AlertTriangle size={20} />,
    },
    MIS_SELLING: {
      title: "Ensure Suitability of Advice",
      description:
        "Implement thorough suitability assessment processes. Document rationale for recommendations and ensure advisers are properly trained.",
      icon: <Target size={20} />,
    },
    CONDUCT: {
      title: "Embed Consumer Duty",
      description:
        "Ensure customer outcomes are at the heart of product design and distribution. Regularly monitor and evidence fair treatment of customers.",
      icon: <Users size={20} />,
    },
    CONFLICTS: {
      title: "Manage Conflicts of Interest",
      description:
        "Identify, prevent, and manage conflicts of interest. Maintain appropriate information barriers and disclosure procedures.",
      icon: <Shield size={20} />,
    },
    PENSIONS: {
      title: "Protect Pension Savers",
      description:
        "Ensure pension transfer advice considers safeguarded benefits. Implement robust due diligence on SIPP investments.",
      icon: <Users size={20} />,
    },
    LENDING: {
      title: "Responsible Lending Practices",
      description:
        "Conduct thorough affordability assessments. Treat borrowers fairly, especially those in financial difficulty.",
      icon: <FileText size={20} />,
    },
    COMPLAINTS: {
      title: "Improve Complaints Handling",
      description:
        "Handle complaints fairly and promptly. Use complaints data for root cause analysis and continuous improvement.",
      icon: <Target size={20} />,
    },
    PRUDENTIAL: {
      title: "Maintain Financial Resilience",
      description:
        "Ensure adequate capital and liquidity buffers. Implement robust financial resource monitoring and stress testing.",
      icon: <TrendingUp size={20} />,
    },
  };
  ...
  return topCategories.map(([category, frequency], index) => {
    const template = lessonTemplates[category] || {
      ...
      description:
        "Review and strengthen controls in this area to align with regulatory expectations.",
