...
import { BreachByTypeChart } from "./BreachByTypeChart.js";
import {
  mockFineRecords,
  emptyRecords,
  createManyRecords,
...
describe("BreachByTypeChart", () => {
  describe("rendering", () => {
    ...
    it("renders metric toggle buttons", () => {
      ...
      expect(
        screen.getByRole("button", { name: "Notices" }),
      ).toBeInTheDocument();
    });
  ...
  describe("interactions", () => {
    it("calls onSelect when legend item is clicked", () => {
      ...
      render(
        <BreachByTypeChart records={mockFineRecords} onSelect={onSelect} />,
      );

      // Find and click a legend item
      const legendItems = screen
        .getAllByRole("button")
        .filter((btn) => btn.classList.contains("panel__legend-item"));
  ...
  describe("help tooltip", () => {
    it("renders help button with tooltip", () => {
      ...
      const helpButton = screen.getByRole("button", {
        name: /Distribution of fines by breach category/,
      });
