...
export function HeroStats({
  ...
  return (
    <section id="hero-section" className="hero hero--dashboard">
      ...
      >
        <div>
          ...
          <h1
            ...
            style={{ fontSize: "var(--text-5xl)", margin: 0 }}
          >
            FCA Enforcement Pulse
          </h1>
          <p className="hero__description">
            Track FCA enforcement actions in real-time. Identify compliance
            risks, learn from peer penalties, and strengthen your firm's
            regulatory posture with insights from 2013 onwards.
          </p>
        </div>
        <div className="hero__actions">
          {onAlertsSubscribe && (
            <button
              type="button"
              className="btn btn-primary hero__subscribe"
              ...
              <div>
                <span className="hero__badge-label">
                  ...
                  <span className="hero__countdown">
                    ({differenceInDays(new Date(), new Date(latest.date))} days
                    ago)
                  </span>
                </span>
                {latest.url ? (
      ...
      <FineVelocityGauge
        timeline={timeline}
        fullTimeline={fullTimeline || timeline}
      />
    </section>
  );
}
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      {chip && (
        <span
          className={`stat-card__chip ${chip.trend === "down" ? "stat-card__chip--down" : ""}`}
        >
...
function FineVelocityGauge({
  ...
  const baseline =
    recentEntries.length > 0
      ? recentEntries.reduce((sum, item) => sum + item.total, 0) /
    ...
    <div className="hero__velocity">
      ...
      <div className="hero__velocity-content">
        ...
        <div className="hero__velocity-meta">
          {change ? (
            <span
              className={`hero__velocity-chip ${change.trend === "down" ? "hero__velocity-chip--down" : ""}`}
            >
