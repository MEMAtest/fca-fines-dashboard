...
import { getSanctions, type CountrySanctions, type SanctionsTier } from "./sanctionsStatus.js";
...
export interface CountryRiskV2Inputs {
  ...
  sourceStates?: Partial<Record<"aml" | "governance" | "sanctions", CountryRiskSourceState>>;
...
export function computeCountryRiskV2(iso2: string, supplied: CountryRiskV2Inputs = {}): CountryRiskV2Result {
  ...
  const sanctions = supplied.sanctions ?? getSanctions(code);
  const assessmentState = supplied.sourceStates?.aml ?? countryRiskSourceStatus("fatf-assessments").state;
  const governanceState = supplied.sourceStates?.governance ?? countryRiskSourceStatus("world-bank-wgi").state;
  const sanctionsState = supplied.sourceStates?.sanctions ?? countryRiskSourceStatus("sanctions-regimes").state;
  const sanctionsCoverageComplete = supplied.sanctionsCoverageComplete ?? sanctionsState === "current";
  ...
  const assessmentAge = assessmentAgeYears(assessment, supplied.asOf ?? new Date());
  ...
  const appliedWeight = (key: (typeof pillarValues)[number][0]) => {
    const item = available.find(([candidate]) => candidate === key);
    return item && availableWeight ? item[2] / availableWeight : 0;
  };

  const floors: CountryRiskFloor[] = [];
  const fatf = getFatfStatus(code);
  if (fatf?.listing === "call-for-action") floors.push({ reason: "fatf-black", minimum: COUNTRY_RISK_FLOORS.fatfBlack });
  if (fatf?.listing === "increased-monitoring") floors.push({ reason: "fatf-grey", minimum: COUNTRY_RISK_FLOORS.fatfGrey });
  if (sanctionsValue.highestTier === "comprehensive") floors.push({ reason: "sanctions-comprehensive", minimum: COUNTRY_RISK_FLOORS.sanctionsComprehensive });
  if (sanctionsValue.highestTier === "sectoral") floors.push({ reason: "sanctions-sectoral", minimum: COUNTRY_RISK_FLOORS.sanctionsSectoral });

  let score: number | null = null;
  if (status !== "insufficient-data") {
    const weighted = available.reduce((sum, [key, value]) => sum + (value as number) * appliedWeight(key), 0);
    score = Math.min(10, round1(Math.max(weighted, ...floors.map((floor) => floor.minimum), 0)));
  }
  ...
  return {
    ...
    band,
    status,
    confidence: confidenceFor([assessmentState, governanceState, sanctionsState], status, assessmentAge),
    pillars,
    floors,
    limitingReasons,
    arithmetic:
      score === null
        ...
        : `${available.map(([key, value]) => `${key} ${value} × ${round1(appliedWeight(key) * 100)}%`).join(" + ")}${floors.length ? `; floor ${Math.max(...floors.map((floor) => floor.minimum))}` : ""} = ${score}`,
  };
}
