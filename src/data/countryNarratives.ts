...
export const NARRATIVES: Record<string, CountryNarrative> = {
  ...
  "GB": {
    ...
    "whyItMatters": [
      "The FCA is the United Kingdom's live RegActions enforcement feed; current action totals are loaded from the source-linked evidence view rather than this narrative snapshot.",
    ...
    "analysis": "The United Kingdom's profile reflects sound governance across all four domains. Voice and accountability (strong) and rule of law and institutions (strong) are the strongest, consistent with a mature legal system and free press. Corruption (strong) is similarly low, supported by a strong CPI reading and an upper-tier standing on the World Bank governance indicators. Political stability (strong) is the comparatively weakest domain but remains low in absolute terms. The United Kingdom is not on the FATF grey or black list and carries no sanctions exposure. The FCA is a live RegActions feed; current action totals and source currency are shown in the enforcement evidence view rather than treated as a static country-risk input.",
