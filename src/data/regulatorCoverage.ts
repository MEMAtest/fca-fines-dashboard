...
const REGULATOR_COVERAGE_SEED: Record<string, RegulatorCoverageSeed> = {
  ...
  FDIC: {
    ...
    note: "Ingested live from the official FDIC Enforcement Decisions & Orders register (11,137 orders after dropping masked/redacted parties; the register spans 1975-2026 but all pre-1989 rows are masked, so published records span 1989-2026). The register is a Salesforce Lightning app; the loader renders it with a headless browser (Playwright/Chromium, fragile lane) and downloads the official 'Download All' CSV export. Each (order × respondent) becomes one record; the civil money penalty is paired per-respondent and fails toward null for prohibition/cease-and-desist orders with no penalty.",
    earliestYear: 1989,
    latestYear: 2026,
    nativeCurrency: "USD",
    defaultCurrency: "GBP",
