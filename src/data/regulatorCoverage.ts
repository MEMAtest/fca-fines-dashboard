...
const REGULATOR_COVERAGE_SEED: Record<string, RegulatorCoverageSeed> = {
  ...
  FDIC: {
    ...
    count: 11137,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the official FDIC Enforcement Decisions & Orders register (11,137 orders after dropping masked/redacted parties; the register spans 1975-2026 but all pre-1989 rows are masked, so published records span 1989-2026). The register is a Salesforce Lightning app; the loader renders it with a headless browser and downloads the official 'Download All' CSV export. Each (order × respondent) becomes one record; the civil money penalty is paired per-respondent and fails toward null for prohibition/cease-and-desist orders with no penalty. Ingestion and public coverage are not yet live.",
    earliestYear: 1989,
    latestYear: 2026,
    nativeCurrency: "USD",
    defaultCurrency: "GBP",
  ...
  AMMC: {
    ...
    count: 10,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the official 'Sanctions pécuniaires' listing (10 monetary sanctions, 2019-2024). The French decision titles are preserved verbatim and the sanctioned party is extracted from them; the monetary figure lives inside each decision PDF, so amounts are null by nature in the listing. Ingestion and public coverage are not yet live.",
    earliestYear: 2019,
    latestYear: 2024,
    nativeCurrency: "MAD",
    defaultCurrency: "EUR",
  ...
  FSS: {
    ...
    count: 39,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the FSS English press-release board filtered to the 'Supervision-Examination' category (39 supervisory/enforcement press releases, 2004-2022; paginated). English press-release titles are preserved verbatim. FSS press releases seldom state a monetary figure in English, so amounts are null by nature. Distinct from the FSC Korea (fsc.go.kr) source. Ingestion and public coverage are not yet live.",
    earliestYear: 2004,
    latestYear: 2022,
    nativeCurrency: "KRW",
    defaultCurrency: "EUR",
