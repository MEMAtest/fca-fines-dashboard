...
const REGULATOR_COVERAGE_SEED: Record<string, RegulatorCoverageSeed> = {
  ...
  TWFSC: {
    ...
    years: "2012-2026",
    count: 929,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the FSC Securities and Futures Bureau administrative-sanction feed (929 sanctions, 2012-2026; 818 carry a parsed NT$ fine amount pulled from each detail page). Detail-page amount extraction is keyword-anchored to the fine itself so unrelated NT$ figures (e.g. mediation amounts) are not mistaken for the penalty. Ingestion and public coverage are not yet live.",
    earliestYear: 2012,
    latestYear: 2026,
    nativeCurrency: "TWD",
    defaultCurrency: "GBP",
  ...
  FRB: {
    ...
    count: 2878,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the official enforcement-actions CSV export (2,878 actions, 1989-2026; ~358 carry a parsed Civil Money Penalty amount, the rest are non-monetary Written Agreements / C&D / prohibitions). Ingestion and public coverage are not yet live.",
    earliestYear: 1989,
    latestYear: 2026,
    nativeCurrency: "USD",
    defaultCurrency: "GBP",
  ...
  SPK: {
    ...
    count: 2252,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the official SPK open-data JSON web service (2,252 administrative monetary penalties, 2016-2026; every record carries a TRY amount). Turkish source text is preserved verbatim. Ingestion and public coverage are not yet live.",
    earliestYear: 2016,
    latestYear: 2026,
    nativeCurrency: "TRY",
    defaultCurrency: "EUR",
  ...
  GHSEC: {
    ...
    count: 65,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the official suspension/revocation/cessation licence tables (65 licence actions, 2019-2022). These are licence enforcement actions rather than monetary penalties, so amounts are null by nature. Ingestion and public coverage are not yet live.",
    earliestYear: 2019,
    latestYear: 2022,
    nativeCurrency: "GHS",
    defaultCurrency: "GBP",
  ...
  IOMFSA: {
    ...
    count: 21,
    dataQuality: "Live loader verified (dry-run)",
    note: "Live loader built and verified against the enforcement-action table (21 civil penalties, 2019-2025; every record carries a GBP amount). The site's WAF rejects plain HTTP, so the loader renders the page with a headless browser (Playwright/Chromium). Ingestion and public coverage are not yet live.",
    earliestYear: 2019,
    latestYear: 2025,
    nativeCurrency: "GBP",
    defaultCurrency: "GBP",
