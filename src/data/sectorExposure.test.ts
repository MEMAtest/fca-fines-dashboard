...
describe("promoted sanctions evidence drives sector exposure", () => {
  it("uses Cuba's promoted comprehensive tier in live sector exposure", () => {
  ...
  it("retains Iran's FATF and comprehensive-sanctions signals", () => {
...
describe("low-risk countries use the complete sanctions zero", () => {
  it("Japan is Low across sectors", () => {
    const rows = sectorsFor("JP");
    expect(rows.every((r) => r.level === "Low")).toBe(true);
  });

  it("Denmark is Low across sectors", () => {
    const rows = sectorsFor("DK");
    expect(rows.every((r) => r.level === "Low")).toBe(true);
  });
});

describe("missing governance remains visible with complete sanctions evidence", () => {
  ...
  it("retains FATF-derived exposure for the British Virgin Islands and reviews governance gaps", () => {
    ...
    expect(row(rows, "Trade & export controls").level).toBe("Low");
    expect(row(rows, "Real estate & luxury assets").level).toBe("Review");
  });
});
