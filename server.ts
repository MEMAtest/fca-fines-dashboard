...
import {
  ...
  getTrends,
  getNotifications,
} from "./server/services/fcaFines.js";
import { getHomepageStats } from "./server/services/homepage.js";
import { getYearlySummary } from "./server/services/yearlySummary.js";
import { submitContactForm } from "./server/services/contact.js";
import {
...
app.get("/api/fca-fines/breach", async (req, res) => {
  try {
    ...
    const limitPenalties = Math.min(
      Number(req.query.limitPenalties || "10"),
      50,
    );
    ...
    if (!data) {
      return res
        .status(404)
        .json({ success: false, error: "Breach not found" });
    }
    res.json({ success: true, data });
  } catch (error) {
...
app.get("/api/fca-fines/sector", async (req, res) => {
  try {
    ...
    const data = await getSectorDetailsBySlug(
      slug,
      limitPenalties,
      limitBreaches,
    );
    if (!data) {
      return res
        .status(404)
        .json({ success: false, error: "Sector not found" });
    }
    res.json({ success: true, data });
  } catch (error) {
...
app.get("/api/homepage/stats", async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch homepage stats" });
  }
});
...
app.get("/api/yearly-summary/:year", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch yearly summary" });
  }
});
